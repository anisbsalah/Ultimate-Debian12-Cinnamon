...
sudo apt install -y /tmp/bulky/packages/*
#sudo dpkg -i /tmp/bulky/packages/*
...
sudo apt-get install -y libavcodec-extra gstreamer1.0-libav gstreamer1.0-plugins-ugly gstreamer1.0-vaapi
...
release="20230712-072601-f4abf8fd"
...
sudo apt install -y atool bat caca-utils elinks epub-utils exiftool ffmpeg ffmpegthumbnailer highlight imagemagick jq jupyter lynx mediainfo mupdf odt2txt openscad rar sed sqlite3 tar transmission unrar unzip w3m w3m-img
