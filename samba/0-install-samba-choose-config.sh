...
if dpkg -s firewalld &>/dev/null; then
	...
	sudo firewall-cmd --permanent --add-service={samba,samba-client,samba-dc,ws-discovery,ws-discovery-client,ws-discovery-tcp,ws-discovery-udp} --zone=home

elif dpkg -s ufw &>/dev/null; then
